...
class WaypointDriver(Node):
    def __init__(self):
        ...
        self.get_logger().info("코너링 정밀 튜닝 완료! 촘촘한 11-Step 웨이포인트 가동!")
    ...
    def image_callback(self, msg):
        ...
        if M_green["m00"] > 0 and M_red["m00"] > 0:
            ...
            if len(track_x) == 0 and len(self.path_queue) == 0:
                ...
                if self.state == 'SEARCHING':
                    ...
                    if len(self.path_queue) == 0:
                        distances = np.sqrt((track_x - rx)**2 + (track_y - ry)**2)
                        # 첫 점 탐색 거리도 조금 당김
                        ring_mask = (distances > 25) & (distances < 45)
                    ...
                    # 11개 확장 루프
                    search_radius = 26  # [튜닝 2] 탐색 범위 축소 (35 -> 26)
                    ...
                    if len(self.path_queue) > 0:
                        ...
                        # [튜닝 3] 도달 판정 거리 축소 (20 -> 15)
                        if math.hypot(target_x - rx, target_y - ry) < 15:
            ...
            if self.state in ['APPROACHING', 'TRACKING']:
                ...
                # 점이 촘촘해졌으므로 조향 민감도 약간 상향 (15도 -> 10도)
